...
describe("workspace store", () => {
  it("toggles document selection and composer attachments together", () => {
    ...
    expect(
      useWorkspaceStore.getState().documentsState.selectedDocumentIds,
    ).toEqual(["src-test"]);
    expect(useWorkspaceStore.getState().chatState.attachedDocumentIds).toEqual([
      "src-test",
    ]);
  });
  ...
  it("updates node positions after constrained drag", () => {
    useWorkspaceStore
      .getState()
      .setNodePosition("node-src-test", { x: 48, y: 42, z: 0 });
    expect(useWorkspaceStore.getState().sceneState.nodes[0]?.position).toEqual({
      x: 48,
      y: 42,
      z: 0,
    });
...
describe("gallery collapsed state", () => {
  ...
  it("defaults to not collapsed", () => {
    expect(useWorkspaceStore.getState().uiShellState.galleryCollapsed).toBe(
      false,
    );
  });
...
describe("theme mode", () => {
  it("defaults to everforest-light", () => {
    expect(useWorkspaceStore.getState().studioState.themeMode).toBe(
      "everforest-light",
    );
  });
