  ...
  return (
    <div className="rounded-2xl border border-[color:var(--panel-border)] bg-[color:var(--surface-2)] p-4">
      <div className="mb-3 flex items-center gap-2.5">
        <div className="h-2.5 w-2.5 rounded-full" style={{ background: document.accent }} />
        <div>
          <p className="text-[11px] uppercase tracking-[0.24em] text-[color:var(--text-kicker)]">{document.preview.headline}</p>
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">{document.title}</h3>
        </div>
      </div>
      ...
      <p className="mt-3 text-sm leading-6 text-[color:var(--text-primary)]">{document.preview.excerpt}</p>
    </div>
  );
}
